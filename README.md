<img src="src\Rhino.Inside.AutoCAD.Applications\Icons\RhinoInsideAutoCADLogo.png" width="128px"/>

# Rhino.Inside® AutoCAD & Rhino.Inside® Civil3d

Rhino.Inside.AutoCAD embeds the full Rhinoceros 3D geometry engine and Grasshopper visual programming environment directly into AutoCAD. Design complex NURBS surfaces, run computational workflows, and exchange geometry bidirectionally - without file exports, coordinate misalignment, or context switching.

Please see the [Rhino.Inside.Autocad Forum](https://discourse.mcneel.com/t/about-the-autocad-category/213088)
to find out more about the project and find out how to get involved.

## Overview

The Rhino.Inside® Technology allows Rhino, Grasshopper, and their add-ons to be
embedded within other products.
This repository is the Rhino.Inside® for Autodesk AutoCAD, named Rhino.Inside.AutoCAD.

## Alpha Testing

This project is in early Alpha testing, and it is not recommended for use in a production environment.

## Supported Versions

This project currently only supports:
- Rhino 8, with AutoCAD 2024, AutoCAD 2025, AutoCAD 2026, AutoCAD 2027 or Civil3d 2024, Civil3d 2025,
  Civil3d 2026, Civil3d 2027
- Rhino 9, with AutoCAD 2025, AutoCAD 2026, AutoCAD 2027 or Civil3d 2025, Civil3d 2026, Civil3d 2027

Rhino 9 requires .NET Core, so it cannot be hosted by AutoCAD 2024 or Civil3d 2024, which run on
.NET Framework.

Autodesk moved the 2025 and 2026 releases from .NET 8 to .NET 10 (AutoCAD 2026.1.2 and the
equivalent 2025 update). Both are supported: the same build runs under either runtime, so no
action is needed when you take that update. Rhino 8 users should be on 8.32 or newer, which is
the first service release Rhino support in a .NET 10 host.

If more than one supported Rhino version is installed, you are asked which one to use the first
time Rhino.Inside.AutoCAD starts in a session. Choose "Always use" to save the choice and skip
the prompt in future sessions.

For full compatibility make sure your applications are up to date.

## Find Out More
More information including a Quick Start Guide can be found on our [Bimorph Website](https://www.bimorph.com/products/rhino-inside-autocad).

# Documentation
See the dedicated docs site here [Documentation](https://www.bimorph.com/products/rhino-inside-autocad/docs).

# Get involved
This project is in active development, so if there are features you would like to see get in touch and we can add them to the road map.
